...
class Cluster(object):
    ...
    def singleLinkageDist(self, other):
        ...
        # initiate the minimum Distance with the first terms
        minDist = self.points[0].distance(other.points[0])
            # if we find a shorter distance among the points, set a new minDist
        for p in self.points:
            for q in other.points:
                if p.distance(q) < minDist:
                    minDist = p.distance(q)
        return minDist
    ...
    def maxLinkageDist(self, other):
        ...
        self and the other point is from other. Uses the 
        Euclidean dist between 2 points, defined in Point."""
        # initiate the maximum Distance with the first terms
        maxDist = self.points[0].distance(other.points[0])
            # if we find a larger distance among the points, set a new maxDist
        for p in self.points:
            for q in other.points:
                if p.distance(q) > maxDist:
                    maxDist = p.distance(q)
        return maxDist
    ...
    def averageLinkageDist(self, other):
        ...
        for p in self.points:
            for q in other.points:
                totDist += p.distance(q)
        # divide it by the number of occurence to get medDist
        medDist = totDist/(len(self.points)*len(other.points))
        return medDist
...
class ClusterSet(object):
    ...
    def findClosest(self, linkage):
        ...
        closest = (self.members[0], self.members[1])
        for c1 in self.members:
            for c2 in self.members:
                if c1 == c2:
                    continue
                elif linkage(c1, c2) < minDist:
    ...
    def mergeOne(self, linkage):
        ...
        # TO DO
        merger = self.findClosest(linkage)
        self.mergeClusters(merger[0],merger[1])
        return merger
...
def test():
    points = buildCityPoints('/Users/bastienbrier/Documents/edX/MIT/6.00.2x/Week 6/Problem Set 6/cityTemps.txt', False)
    hCluster(points, Cluster.singleLinkageDist, 10, False)
    #points = buildCityPoints('/Users/bastienbrier/Documents/edX/MIT/6.00.2x/Week 6/Problem Set 6/cityTemps.txt', True)
