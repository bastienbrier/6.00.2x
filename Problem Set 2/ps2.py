...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    for trial in range(num_trials):
        ...
        # create a list of all the robots
        listRobots = []
        
        # add the robots to the list
        for n in range(num_robots):
            listRobots.append(robot_type(room,speed))
        
        # initializes the number of steps
        steps = 0
        
        # as long as the room is too dirty, update all the robots
        while (float(room.getNumCleanedTiles()) / float(room.getNumTiles())) < min_coverage:
            for robot in listRobots:
                robot.updatePositionAndClean()
            steps += 1
        
        # append the number of tests to the list
        listTrials.append(steps)
    
    # return the mean
    return float(sum(listTrials)) / len(listTrials)
...
#     plot.
#
#    Plot1 = showPlot1("Time It Takes 1 - 10 Robots To Clean 80% Of A Room", "Number of Robots", "Time-steps")
#
...
#     plot.
#
#    Plot2 = showPlot2("Time It Takes Two Robots To Clean 80% Of Variously Shaped Rooms", "Aspect Ratio", "Time-steps")
#
